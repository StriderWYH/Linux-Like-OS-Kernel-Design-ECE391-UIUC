...
void page_init(){
    ...
    PDE_TABLE[0].KBPDE.user_or_Supervisor = 1;
    PDE_TABLE[0].KBPDE.write_through = 0;
    PDE_TABLE[0].KBPDE.table_base_add = ((int32_t)&(PTE_TABLE[num_PTE - 1])) >> 12;           // set the first PDE entry, it should be KBPDE, and set the arguement. 12 means the table base is the highest 20bit
    ...
    PDE_TABLE[1].MBPDE.table_base_add = ((kernel * 2) - 1) >> 22;     //set the PDE of kernel MBPDE, it should be sudo view and is present, 22 is 22 bit that the address need to shift because the base address is only 10 bit
    ...
    for (PDE_index = 2; PDE_index < num_PDE; PDE_index++){
        ...
        PDE_TABLE[PDE_index].MBPDE.table_base_add = (((PDE_index + 1) * size_of_4mb) - 1) >> 22;
        }
    ...
    for (PTE_index = 0; PTE_index < num_PTE; PTE_index++){
        ...
        PTE_TABLE[PTE_index].table_base_add = (((PTE_index + 1) * size_of_4kb) -1) >> 12;
        if(PTE_index * size_of_4kb == video_memory){
          PTE_TABLE[PTE_index].present = 1;  
          PTE_TABLE[PTE_index].user_or_Supervisor = 0;     //set the PTE if the PTE is the video memory. Then the memory is used so set its present to 1 and it is also sudo view
        }
    }   
